...
.i16
_028A:
        stz a:zTargetIndex        ;**optimize: wastes a byte
        ldx #$0133
...
:        lda ROMEffectInfo,X
        ...
        tdc
        jsr SetBit_X
        sta zPartyTargets        ;default to targetting self
        bra TargetSet
Targetting:
        and #$08                ;target enemy
        bne TargetEnemy
        lda #$F0
        sta zPartyTargets        ;entire party
        bra TargetSet
TargetEnemy:
        lda Temp
...
TargetAll:
        ...
        sta zMonsterTargets
TargetSet:
        stz zTempAttachedSpell        ;CastSpell routine params
        stz zTempSkipNaming
...
DeadAlly:
        ...
        jsr SetBit_X
SetAlly:
        sta zPartyTargets        ;target single dead ally
        bra TargetSet
SingleEnemy:
        tdc
        tax
...
TargetAll:
        ...
        bne AllEnemy
        lda #$F0
        sta zPartyTargets        ;all allies
        bra TargetSet
AllEnemy:
        lda #$FF
        sta zMonsterTargets        ;all enemies
TargetSet:
        stz zTempAttachedSpell        ;params for CastSpell
        stz zTempSkipNaming
        jsr CastSpell
        lda zTempAttachedSpell
        beq Ret
        lda zTempAttachedSpell        ;second spell, for phoenix summon
        sta TempSpell
        stz TempIsEffect
        lda zTempMonsterTargets
        sta zMonsterTargets
        lda zTempPartyTargets
        sta zPartyTargets
        inc zTempSkipNaming        ;2nd spell has no label and diff anim
        jsr CastSpell
Ret:        rts
...
:        lda TempMagicInfo,X
        ...
        tdc
        jsr SetBit_X
        sta zPartyTargets        ;default to attacker if no targetting
        bra TargetSet
Targetting:
        and #$40                ;all targets
        bne TargetAll
...
:            lda ROMEffectInfo,X
        ...
        bne Targetting
        lda zAttackerIndex        ;default to attacker
        tax
        tdc
        jsr SetBit_X
        sta zPartyTargets
...
:            jsr FindOpenGFXQueueSlot
        ...
        jsr GFXCmdDamageNumbers
        lda #$FF
        sta a:zMonsterTargets        ;**optimize: wasted bytes
        stz a:zPartyTargets
        lda TempEffect
        sta TempSpell
        lda #$01
...
_24FD:
        ...
        ldx zAttackerOffset        ;not actually attacker, in this case
        jsr HasteSlowMod
        sta CurrentTimer,Y
        sta InitialTimer,Y
        lda #$01
...
_2521:
        jsr AddTimerOffsetY      ;Y = X + zTimerOffset
        txa
        asl
        clc
...
AITargetMonstersExcept:
        ...
        ldx #CharStruct::_4 - CharStruct        ;first monster offset
Loop:
        ldy $10
...
;special commands (Flag byte $FD)
        lda (zAIOffset),Y        ;copy command byte
        sta AIBuffer,X
        iny
        inx
        cmp #$F7
        ...
        cmp #$F0
        bcs OtherSpecial
        lda (zAIOffset),Y        ;copy 2 data bytes
        sta AIBuffer,X
        iny
        inx
        lda (zAIOffset),Y
...
CopyExtraBytes:        ;F7 command, copies 2 bytes then extra bytes depending on the first (min 1)
        lda (zAIOffset),Y        ;number of extra data bytes to copy
...
_329C:
        ...
        ror $16
        lsr
        ror $16                ;low 4bits of zMMOffset in high 4 of $16
        lda $16
...
WipeAIScript:                ;wipes 20 bytes of MonsterAIScript
        ...
        bne WipeAIScript
        longa
        dec zAIScriptOffset   ;unsure why, incremented at end anyway
        shorta
        ldx zMMTargetOffset
...
_334D:
        ...
        lda AIBuffer+3,Y
        sta $2722
        inc zAIBufferOffset        ;incrememnt buffer to account
        inc zAIBufferOffset        ;.. for the extra 3 bytes
        inc zAIBufferOffset
        tdc
        tax
        lda #$02
...
_34D9:
        ...
        sta zAIBufferOffset        ;.. last byte is in called routine
        rts
...
NormalTargetting:
        ...
        bne Enemy
        lda zTempCharm        ;flag for charm, inverts targetting
        bne RandomParty
RandomMonster:
        tdc
        tax
...
CounterAttempt2:        ;50% chance to counter monster fight or damaging specialty
        ...
        sta CharStruct::Command,Y
        sec
        lda ActedIndex        ;first counter check used zAttackerIndex
        sbc #$04        ;to monster index
        tax
        tdc
        jsr SetBit_X
...
_3D08:
        ...
        tdc
        tay
        ldx zAttackerOffset        ;offset of forced target
:       lda SavedAction2,Y
        sta CharStruct::ActionFlag,X
        inx
        iny
...
CopyOneChar:
        ...
        lda wEncounterIndex+1        ;1 for boss fights
        beq DoneLenna
        lda wEncounterIndex
        cmp #$BA                ;Forza/Magisa
        bne DoneLenna
...
CheckMagicBit:
        ...
        bcs CheckBlue
        lda #$FF
        sta CharSpells::_0::ID,x
        sta CharSpells::_1::ID,x
        sta CharSpells::_2::ID,x
        sta CharSpells::_3::ID,x
        bra NextSpell
CheckBlue:
        txa
...
SetupSpell:
        ...
        sta CharSpells::_0::Level,x
        sta CharSpells::_1::Level,x
        sta CharSpells::_2::Level,x
        sta CharSpells::_3::Level,x
        lda CharSpells::ID,x
        longa
        jsr ShiftMultiply_8
        tax
        ...
        and #$7F        ;just MP Cost
        ldx $20
        sta CharSpells::_0::MP,x
        sta CharSpells::_1::MP,x
        sta CharSpells::_2::MP,x
        sta CharSpells::_3::MP,x
        pla
        sta CharSpells::_0::Targetting,x
        sta CharSpells::_1::Targetting,x
        sta CharSpells::_2::Targetting,x
        sta CharSpells::_3::Targetting,x
NextSpell:
...
:       lda #$0A        ;C1 Routine: execute graphics script
        ...
        lda EncounterInfo::IntroFX
        bpl :+
        lda zAttackerIndex      ;this section for credits demo
        cmp #$04                ;monster check
        bcs :+
        lda #$20
...
_4923:
        ...
        bpl Finish
        lda ActionAnim0::TargetBits
        ora ActionAnim1::TargetBits
        beq :+
        lda ActionAnim0::OrigTargetBits
        sta ActionAnim0::TargetBits
:        tdc
        tax
ShiftActionAnims:
        lda ActionAnim2::Flags,x
        sta ActionAnim1::Flags,x
        inx
        ...
        beq Ret
        lda ActionAnim1::TargetBits
        ora ActionAnim2::TargetBits
        beq Finish
        lda ActionAnim1::OrigTargetBits
        sta ActionAnim1::TargetBits
Finish:
        tdc
        tax
ShiftActionAnims2:
        lda ActionAnim3::Flags,x
        sta ActionAnim2::Flags,x
        inx
        cpx #$0023        ;35 bytes, which is 5 ActionAnim structures
...
:       lda CharStruct::Status2,X
        ...
        lda #$80
        sta CharStruct::ActionFlag,X
:       cpx #$0200                            ;zAttackerOffset is a monster
        bcs ActionReady
        lda CharStruct::Command,X
        beq ActionReady
        cmp #$25                        ;Hide
...
SingleTarget:
        lda zMonsterTargets        ;monsters 0-7
        pha
        and #$F0                ;select monsters 0-3
        jsr ShiftDivide_16        ;shift into slots 4-7
        ora zPartyTargets        ;party into slots 0-3
...
SetMonsterTargets:
        ...
        jsr SetBit_X
        sta zMonsterTargets
Finish:       plx                         ;original zAttackerOffset
        stx zAttackerOffset
        rts
...
:       lda FieldData,X        ;copy data like Escape count and battle event flags
        ...
        sta CharStruct::_4::CharRow
        sta CharStruct::_5::CharRow
        sta CharStruct::_6::CharRow
        sta CharStruct::_7::CharRow
        sta CharStruct::_8::CharRow
        sta CharStruct::_9::CharRow
        sta CharStruct::_10::CharRow
        sta CharStruct::_11::CharRow
        rts
...
:       lda CharStruct::CharRow,X
        ...
        bne CheckBackAttack
        lda CharStruct::_0::Passives2
        ora CharStruct::_1::Passives2
        ora CharStruct::_2::Passives2
        ora CharStruct::_3::Passives2
...
_5070:
        jsr WipeDisplayStructures
        ldx #wFieldItemsWon::SIZE - 1
:       stz wFieldItemsWon,X
...
_515C:
        ...
        stx $0E
CopyBaseStats:
        lda wFieldChar::Level,Y
        sta CharStruct::Level,X
        lda wFieldChar::BaseStr,Y
        sta CharStruct::BaseStr,X
        lda wFieldChar::BaseAgi,Y
        sta CharStruct::BaseAgi,X
        lda wFieldChar::BaseVit,Y
        sta CharStruct::BaseVit,X
        lda wFieldChar::BaseMag,Y
        sta CharStruct::BaseMag,X
        jsr NextCharOffset
        longa
        tya
        clc
        adc #wFieldChar::ITEM_SIZE
        tay
        shorta0
        inc $0E
        lda $0E
        cmp #wFieldChar::NUM_ITEMS
        bne CopyBaseStats
        tdc
        tax
        stx $0E
...
SearchLoop:
        ...
        bne NextSearchItem
        clc
        lda wFieldItemsQty,X        ;match, merge quantities into base item
        adc wFieldItemsQty,Y
        cmp #$63
        bcc :+
        lda #$63                ;max 99
:       sta wFieldItemsQty,X
        tdc                         ;clear dupe's old slot
        sta wFieldItems,Y
        sta wFieldItemsQty,Y
NextSearchItem:
        iny
        cpy #$0100
        bne SearchLoop
...
:       sta wFieldItemsQty,X
        bne :+
        sta wFieldItems,X                ;Empty slot for Qty 0
:        DEX
        bpl CopyInventory
...
:       sta MonsterKillCount
        ...
        stx VictoryExp+1
        stx zTempMonsterIndex
        ldy #$0200                ;first monster offset
        sty zTempMonsterOffset
TallyLoot:        ;gil, exp and drops
        ldx zTempMonsterIndex        ;monster index
        lda MonsterKillTracker
        jsr SelectBit_X
        beq NextLoot
        clc
...
NextLoot:
        ...
        shorta0
        sty zTempMonsterOffset
        inc zTempMonsterIndex        ;monster index
        lda zTempMonsterIndex
        cmp #$08                ;8 monsters
        bne TallyLoot
        clc
        lda Gil
...
:       lda VictoryExp
        sta MessageBoxData::Var2+0                ;data slots are hardcoded tho
        lda VictoryExp+1                ;..maybe they fix it later
        sta MessageBoxData::Var2+1
        lda VictoryExp+2
        sta MessageBoxData::Var2+2
        lda VictoryExp
        ora VictoryExp+1
        ora VictoryExp+2
        beq :+
...
:       lda EncounterInfo::AP
        beq :+
        sta MessageBoxData::Var3
        stz MessageBoxData::Var3+1
        stz MessageBoxData::Var3+2
        jsr CheckHideAP
...
:        ASL
        ...
        sta MessageBoxData::Var2
        lda #$33                ;alternate ability learned message
        sta MessageBoxes        ;..which shows magic level
:       stz MessageBoxData::Var1+1        ;clear high bytes of message box data
        stz MessageBoxData::Var1+2
        stz MessageBoxData::Var2+1
        stz MessageBoxData::Var2+2
        lda #$0A                ;c1 routine
        jsr CallC1
        jmp JobLevelLoop
NextJobCheck:
...
Normal:
        ...
        sta $0E                        ;random roll
        lda zTempMonsterIndex        ;monster index
        asl
        tax
        longa
        lda BattleMonsterID,X
...
:       lda BattleItemsWon,X
        sta wFieldItemsWon,X
        inx
        cpx #wFieldItemsWon::SIZE
        bne :-
        rts
...
Loop:
        ...
        sta CharStruct::CharRow,Y
        lda z70        ;seems to have bits set for front row monsters
        jsr SelectBit_X
        bne Next
        lda CharStruct::CharRow,Y
...
CastSpell1:
        stz TempIsEffect
        jsr CastSpell
        lda zTempAttachedSpell        ;attached spell for phoenix
        beq CastSpell2
        lda zTempAttachedSpell
        sta TempSpell
        stz TempIsEffect
...
CastSpell2:
        ...
        stz TempIsEffect
        jsr CastSpell
        lda zTempAttachedSpell        ;attached spell for phoenix fire
        beq Finish
        lda zTempAttachedSpell
        sta TempSpell
        stz TempIsEffect
...
:       lda MonsterAIScript,X
        ...
        adc #MonsterMagic
        tax
        stx zMMOffset                ;MonsterMagic offset
        lda $0E
        asl                 ;monster index *32
        clc
        adc #MMTargets
        tax
        stx zMMTargetOffset        ;MonsterMagic Targets offset
        shorta0
CastNextMonsterSpell:
        lda $1E
        tay
        lda (zMMOffset),Y        ;MonsterMagic
        cmp #$FF
        beq Ret
        sta TempSpell
        cmp #$E9        ;banish
...
:       stz zTempAttachedSpell
        ...
        asl
        tay
        lda (zMMTargetOffset),Y        ;MonsterMagic Party Targets
        sta zPartyTargets
        iny
        lda (zMMTargetOffset),Y        ;MonsterMagic Monster Targets
        sta zMonsterTargets
        stz TempIsEffect
        jsr CastSpell
        inc $1E
...
_5F75:
        sta zTempNumHits        ;number of hits        (-1?)
        lda TempIsEffect
        beq NormalMagic
...
CopyAttacks:                ;one copy into AttackInfo table for each zTempNumHits (which is actual hits -1)
...
ProcessHits:                ;sets up targetting and attack type for zTempNumHits + 1 attacks
        lda zMonsterTargets
        beq Party
        tdc
        tax
...
Phoenix:
        lda zMonsterTargets
        sta zTempMonsterTargets
        lda zPartyTargets
        sta zTempPartyTargets
        lda #$FF        ;all targets
        sta zMonsterTargets
        stz zPartyTargets
        lda #$70        ;phoenix (life portion)
        sta zTempAttachedSpell
        ldx zAttackerOffset
        bra Finish
Odin:
        tdc
...
ClearCommandLoop:
        ...
        bpl AfterMiss
        ldx #$4000
        stx DamageToTarget        ;if zAtkMissed bit 80h set, sets a flag
        bra AfterMiss
NotMiss:
        lda zAtkElement
...
:        LDA zParam1
        ...
        sta MessageBoxData::Var2+0,Y
        sta MessageBoxData::Var2+1,Y
        sta MessageBoxData::Var2+2,Y
        sta MessageBoxData::Var3+0,Y
        sta MessageBoxData::Var3+1,Y
        sta MessageBoxData::Var3+2,Y
        bra :++
:        LDA CharStruct::CurHP,X
        sta MessageBoxData::Var2+0,Y
        lda CharStruct::CurHP+1,X
...
;Param2: Spell Power
;
;**optimize: save a byte on zAtkMissed, use 16 bit mode
.proc Attack23
...
_6FC9:
        ...
        bne Miss
        lda zParam2
        sta zParam1                ;move Crit% to zParam1
...
;**optimize: Don't load zParam2 twice
.proc Attack40
...
;Check for Hit
;Rolls checks against zHitPercent($4E) and zEvadePercent($4F)
.proc CheckForHit
...
_8261:
        ...
        lda zParam2
        jsr Random_X_A
        sta zAttack        ;0..Param2
        clc
        lda zAttack
        adc zParam2
        sta zAttack        ;Param2 + 0..Param2
        tdc
        adc #$00
        sta $51
        lda Level
...
_8299:
        ...
        clc
        adc $0E
        sta zAttack            ;Item Attack + 0..Item Attack / 8
        tdc
        adc #$00
        sta zAttack+1
        lda Strength
...
_830A:
        ...
        bcs GilOK
        tdc
        sta zAttack        ;Not enough Gil, Attack = 0
        shorta
        bra Finish
GilOK:  tdc
        shorta
...
:        ;;Elemental Magic Sword L3 Instant Death
        lda CharStruct::Status1,X                ;X is still zTargetOffset
        and #$02                                     ;zombie
        bne Return
        lda CharStruct::Status1,X
        ora #$80                                ;dead
...
:        ;;Elemental Magic Sword L2 vs Weakness
        ...
        and CharStruct::MSwordElemental2,X          ;X is still zTargetOffset
        beq :+
        longa
        lda zAttack
        asl
...
:        ;;Elemental Magic Sword L1 vs Weakness
        lda $0E                                        ;CharStruct::EWeak for Target
        and CharStruct::MSwordElemental1,X          ;X is still zTargetOffset
        beq Return
        longa
        asl zAttack
        stz zDefense
...
_876E:
        lda zMagicSword                ;shouldn't ever be set
...
:        ;check half
        ...
        and zAtkElement
        beq :+
        lsr zParam2    ;*halves param2 instead of Attack ($50)
        rts
:        ;check weak
        lda CharStruct::EWeak,X
        and zAtkElement
        beq Ret
        asl zParam2            ;*doubles param2 instead of Attack
                        ;*also doesn't 0 def
Ret:        rts
...
_87DF:
        lda zMagicSword        ;no crits with magic sword,
...
:        ;Unsure what this section is for, attacker takes damage but no target healing
        lda zAttackerDamaged
        beq :++
        ldx zAttackerOffset                        ;uses attacker's defense instead
        lda CharStruct::Defense,X
        tax
        stx zDefense
        jsr CalcBaseDamage
...
;Damage is zParam2($58) * Current HP of target / 16 unless target is Heavy
.proc GravityDamage
...
_8A05:
        ...
        beq :+
        longa
        asl zM                      ;would have doubled damage
        shorta
:       lda zAtkHealed
        beq :+
        ldx BaseDamage
        stx HealingToTarget
        rts
:       lda zAttackerDamaged        ;attacker damaged instead of target
        beq Finish
        ldx zAttackerOffset
...
_8BCF:
        lda zParam3
        pha                 ;Save zParam3 to restore later
        ldx zAttackerOffset
        ...
        stz zAtkMissed
        pla
        sta zParam3        ;Restore Original zParam3
        stz StatusFixedDur
        rts
...
;Status Hit% in zParam2/$58
;High bit set means Status1, unset means Status2
;Status to apply in zParam3/$59
;
...
_8C2F:
        stz zParam3                ;used for status here
        ldx zAttackerOffset
        lda CharStruct::Specialty,X
...
_8CAC:
        ...
        lda zTargetIndex
        jsr ResetATB
        ldx zAttackerOffset                ;ResetATB actually puts target here
        lda #$80
        sta CharStruct::ActionFlag,X
        stz CharStruct::Command,X
        lda zTargetIndex
...
:
        ...
        stz CharStruct::CurHP+1,X
        plx                                 ;Attacker Offset
        stx zAttackerOffset                ;Restore original Attacker
        bra :+
NotZombie:
        lda zTargetIndex
        cmp #$04                        ;Slots 4 and up are monsters
...
:
        ...
        bne Miss
        lda CharStruct::Status1,X
        ora zParam3                     ;Set any other status
        sta CharStruct::Status1,X
:
...
:        STA UncontrolledATB,X
        ...
        sta CurrentTimer::ATB,Y
        plx
        stx zAttackerOffset        ;Restore attacker variable
        bra :++
...
;Remove Status 1
;Status in zParam3, unset bits are to be cleared
.proc RemoveStatus1
...
_ResetATB:                        ;need to update ATB if Zombie/Stone was set then cleared
        ...
        phx
        lda zTargetIndex
        jsr ResetATB                ;changes zAttackerOffset
        plx
        stx zAttackerOffset        ;restore original
        bra RemoveStatus
TestStone:
        lda zParam3
        eor #$FF
...
:        LDA zTargetIndex
        ...
        ldx zAttackerOffset        ;is now target after ResetATB
        lda #$80
        sta CharStruct::ActionFlag,X
        stz CharStruct::Command,X
        lda zTargetIndex
...
;Starts timer if zParam3 poison bit is set
.proc StartStatus1Timer

_92A5:
        lda zParam3                ;Status1 here
        and #$04                ;Poison
        beq Return
        lda #$01
        tax
...
;Stops timer if zParam3 poison bit is set
...
:        JSR FindFirstTargetPlus1
        clc
        adc zTargetIndex                ;dunno where it's set if not -1
        sta zTargetIndex
        sta AnotherTargetIndex
        jsr CollectMPCost
Finish:
...
FindTargetLoop:
        ...
        bne FindTargetLoop
        lda zAttackerIndex         ;defaults to attacker
        tay
Found:        TYA
        rts
...
:        TAX                         ;now either party or monster index
        ...
        sta zTargetIndex
        tax
        stx zTargetOffset        ;give up and zap the first party member
        bra DoneReflect
PickRandomTarget:
        ...
        bne PickRandomTarget
        lda $10
        sta zTargetIndex        ;new target index
        cmp #$04                ;monster check
        bcc :+
        sec
...
FindCoverLoop:        ;searches for party members that can cover, stores their indexes at $2620
        ...
        ldy $10
        lda $0E
        cmp zTargetIndex                ;can't cover yourself
        beq Next
...
NextHP8b:
        ...
        sta zTargetIndex        ;now the new target
        sta AnotherTargetIndex
        longa
        jsr ShiftMultiply_128
        tax
        ...
        stx zAttackerOffset        ; not sure if it was ever changed?
Ret:        rts
